...
import random
import config
from data_utils import normalizeString,inputVar,outputVar
...
class Corpus(object):
    def __init__(self,conv_path,doc_path,min_freq=2,max_history_len=200,max_response_len=30,
                      embed_file=None):
        ...
        prepared_vocab_file = 'vocab' + str(min_freq) + '.pt'

        self.prepared_data_file = os.path.join(conv_path, prepared_data_file)
        self.prepared_vocab_file = os.path.join(conv_path, prepared_vocab_file)
    ...
    def read_data(self,data_type="train"):
        ...
        for file in os.listdir(dataPath):
            ...
            for i in df.index:
                pair = {}
                pair['doc']= self.wiki_strings[df.iloc[i].wikiIdx][df.iloc[i].docIdx]
    ...
    def build_examples(self, data):
        ...
        try:
            ...
            doc_inp, doc_lengths = inputVar(doc_batch, self.vocab)
            inp, lengths = inputVar(input_batch, self.vocab)
            output, mask, max_target_len = outputVar(output_batch, self.vocab)
